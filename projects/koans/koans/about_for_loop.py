...
def test_for_loop_with_enumerate():
    """
        В python есть встроенная функция enumerte, которая позволяет получить доступ к индексу последовательности, возвращая при каждом проходе цикла индекс и значение переменной.
